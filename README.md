...
The code is designed to be educational, with only 5 files and under 1000 lines of pure PyTorch
...
| ![Image 7](assets/ski.png)    | ![Image 8](assets/stadium.png)  |
...
To make these predictions, language models like GPT-2 first convert the prompt into text or "word" embeddings. These embeddings can be thought of as points in a high-dimensional space where similar words are near one another
...
[OpenAI CLIP](https://openai.com/index/clip/) is a model that finds similarities between images and text by aligning them in a shared vision-language embedding space. For example, CLIP transforms images so that the embedding of a cat image is close to text embeddings of phrases about cats
